...
#include "Renderer/StorageBufferSet.hpp"
...
namespace Vanta {
	...
	public:
		...
		{
			VA_CORE_ASSERT(m_StorageBuffers.find(frame) != m_StorageBuffers.end());
			VA_CORE_ASSERT(m_StorageBuffers.at(frame).find(set) != m_StorageBuffers.at(frame).end());
			VA_CORE_ASSERT(m_StorageBuffers.at(frame).at(set).find(binding) != m_StorageBuffers.at(frame).at(set).end());
