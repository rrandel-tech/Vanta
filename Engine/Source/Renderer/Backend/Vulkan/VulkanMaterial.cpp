...
namespace Vanta {
	...
	void VulkanMaterial::SetVulkanDescriptor(const std::string& name, const Ref<Texture2D>& texture)
	{
		...
		VA_CORE_ASSERT(wds);
		m_ResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(PendingDescriptor{ PendingDescriptorType::Texture2D, *wds, {}, texture.As<Texture>(), nullptr }));
	...
	void VulkanMaterial::SetVulkanDescriptor(const std::string& name, const Ref<TextureCube>& texture)
	{
		...
		const VkWriteDescriptorSet* wds = m_Shader.As<VulkanShader>()->GetDescriptorSet(name);
		VA_CORE_ASSERT(wds);
		m_ResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(PendingDescriptor{ PendingDescriptorType::TextureCube, *wds, {}, texture.As<Texture>(), nullptr }));
		m_PendingDescriptors.push_back(m_ResidentDescriptors.back());
	}
	...
	void VulkanMaterial::SetVulkanDescriptor(const std::string& name, const Ref<Image2D>& image)
	{
		...
		const VkWriteDescriptorSet* wds = m_Shader.As<VulkanShader>()->GetDescriptorSet(name);
		VA_CORE_ASSERT(wds);
		m_ResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(PendingDescriptor{ PendingDescriptorType::Image2D, *wds, {}, nullptr, image.As<Image>() }));
		m_PendingDescriptors.push_back(m_ResidentDescriptors.back());
	}
	...
	void VulkanMaterial::UpdateForRendering()
	{
		...
		Renderer::Submit([instance, writeDescriptors, residentDescriptors, pendingDescriptors]() mutable
		{
			...
			for (auto& descriptor : residentDescriptors)
			{
				if (descriptor->Type == PendingDescriptorType::Image2D)
				{
					...
					{
						VA_CORE_WARN("Found out of date Image2D descriptor ({0} vs. {1})", (void*)image->GetImageInfo().ImageView, (void*)descriptor->WDS.pImageInfo->imageView);
			...
			if (writeDescriptors.size())
			{
				...
				VA_CORE_WARN("Updating {0} descriptor sets", writeDescriptors.size());
		...
		Renderer::Submit([instance]() mutable
		{
			...
			if (instance->m_WriteDescriptors.size())
			{
				...
				VA_CORE_WARN("VulkanMaterial - Updating {0} descriptor sets", instance->m_WriteDescriptors.size());
