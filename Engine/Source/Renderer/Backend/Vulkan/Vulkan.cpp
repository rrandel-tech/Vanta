...
namespace Vanta::Utils {
	...
	void RetrieveDiagnosticCheckpoints()
	{
		...
		}
		{
			...
			uint32_t retrievedCount = checkpointCount;
			vkGetQueueCheckpointDataNV(::Vanta::VulkanContext::GetCurrentDevice()->GetComputeQueue(), &retrievedCount, data);
			VA_CORE_ERROR("RetrieveDiagnosticCheckpoints (Compute Queue):");
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
				VA_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
