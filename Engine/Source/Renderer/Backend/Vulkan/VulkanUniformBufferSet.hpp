...
namespace Vanta {
	...
	public:
		...
		{
			VA_CORE_ASSERT(m_UniformBuffers.find(frame) != m_UniformBuffers.end());
			VA_CORE_ASSERT(m_UniformBuffers.at(frame).find(set) != m_UniformBuffers.at(frame).end());
			VA_CORE_ASSERT(m_UniformBuffers.at(frame).at(set).find(binding) != m_UniformBuffers.at(frame).at(set).end());
