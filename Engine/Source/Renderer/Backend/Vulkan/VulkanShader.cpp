...
namespace Vanta {
	...
	static std::string ReadShaderFromFile(const std::string& filepath)
	{
		...
		{
			VA_CORE_VERIFY(false, "Could not load shader!");
		}
		in.close();
		return result;
	}
	...
	{
		VA_CORE_ASSERT(set < m_DescriptorSetLayouts.size());
		ShaderMaterialDescriptorSet result;
	...
	{
		VA_CORE_ASSERT(set < m_ShaderDescriptorSets.size());
		VA_CORE_ASSERT(m_ShaderDescriptorSets[set]);
