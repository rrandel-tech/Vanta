...
namespace Vanta {
	...
	void VulkanSwapChain::InitSurface(SDL_Window* windowHandle)
	{
		...
		VA_CORE_ASSERT(result, "SDL_Vulkan_CreateSurface failed");
		...
		VA_CORE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		VA_CORE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync)
	{
		...
		VA_CORE_ASSERT(presentModeCount > 0, "");
