...
namespace Vanta {
	...
	static GLenum VantaToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
			case Vanta::TextureFormat::RGB:     return GL_RGB;
			case Vanta::TextureFormat::RGBA:    return GL_RGBA;
		}
		return 0;
	}
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		VA_RENDER_1(self, {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, VantaToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, VantaToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		VA_RENDER_S1(srgb, {
			...
			if (srgb)
			{
				...
				VA_CORE_INFO("Creating srgb texture width {0} mips", levels);
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, VantaToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : VantaToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
				glGenerateMipmap(GL_TEXTURE_2D);
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		VA_RENDER_S({
			glTextureSubImage2D(self->m_RendererID, 0, 0, 0, self->m_Width, self->m_Height, VantaToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
		});
	}

	void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
	{
		VA_CORE_ASSERT(m_Locked, "Texture must be locked!");
		...
		: m_FilePath(path)
	{
		...
		VA_RENDER_S3(faces, faceWidth, faceHeight, {
			glGenTextures(1, &self->m_RendererID);
			...
			auto format = VantaToOpenGLTextureFormat(self->m_Format);
