...
namespace Vanta {
		...
		: m_FilePath(filename)
	{
		...
		m_Material.reset(new Vanta::Material(m_MeshShader));
	...
	void Mesh::TraverseNodes(aiNode* node, int level)
	{
		...
		for (int i = 0; i < level; i++)
			levelText += "-";
		VA_CORE_TRACE("{0}Node name: {1}", levelText, std::string(node->mName.data));
	...
	void Mesh::Render(Timestep ts, const glm::mat4& transform, Ref<MaterialInstance> materialInstance)
	{
		...
		VA_RENDER_S2(transform, materialOverride, {
